...
Start-Process -wait -FilePath $PSScriptRoot\setup-x86_64.exe -ArgumentList "--root=F:\Cygwin --no-startmenu --local-package-dir=F:\Downloads --site=http://cygwin.mirror.constant.com --packages autoconf,automake,ccache,gcc-g++,git,libtool,make,gawk,libexpat-devel,libxml2-devel,python36,python36-future,python36-lxml,python36-pip,libxslt-devel,python36-devel,procps-ng,zip,gdb,ddd,xterm --quiet-mode"
Start-Process -wait -FilePath "F:\Cygwin\bin\bash" -ArgumentList "--login -i -c 'ln -s /usr/bin/python3.6 /usr/bin/python'"
Start-Process -wait -FilePath "F:\Cygwin\bin\bash" -ArgumentList "--login -i -c 'ln -s /usr/bin/pip3.6 /usr/bin/pip'"

Write-Output "Downloading extra Python packages (4/5)"
Start-Process -wait -FilePath "F:\Cygwin\bin\bash" -ArgumentList "--login -i -c 'pip install empy pyserial pymavlink'"
